...
if ( ! class_exists( 'Understrap_WP_Bootstrap_Navwalker' ) ) {
	...
	class Understrap_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		 ...
		 *
		 * Supported linkmods: .disabled, .nav-header, .dropdown-header, .dropdown-divider, .sr-only
		...
		private function seporate_linkmods_and_icons_from_classes( $classes, &$linkmod_classes, &$icon_classes, $depth ) {
			...
			foreach ( $classes as $key => $class ) {
				...
				if ( preg_match( '/^disabled|^sr-only/i', $class ) ) {
					...
					$linkmod_classes[] = $class;
					unset( $classes[ $key ] );
				} elseif ( preg_match( '/^nav-header/i', $class ) ) {
					// Test for .nav-header or .dropdown-divider 
					$linkmod_classes[] = $class;
			...
			if ( ! empty( $linkmod_classes ) ) {
				foreach ( $linkmod_classes as $link_class ) {
					if ( ! empty( $link_class ) ) {
						...
						} elseif ( 'nav-header' === $link_class ) {
							$linkmod_type = 'nav-header';
				...
				foreach ( $linkmod_classes as $link_class ) {
					if ( ! empty( $link_class ) ) {
						...
						if ( 'disabled' === $link_class ) {
							...
							$atts['href'] = '#';
							unset( $atts['target'] );
						} elseif ( 'dropdown-header' === $link_class || 'dropdown-divider' === $link_class || 'dropdown-item-text' === $link_class || 'nav-header' === $link_class ) {
